...
class SupervisionRepository(BaseRepository[Supervision]):
    ...
    def get_reponedores_by_supervisor(self, db: Session, supervisor_id: int, id_empresa: int) -> List[Usuario]:
    ...
    def get_reponedores_disponibles(self, db: Session, id_empresa: int) -> List[Usuario]:
        """Obtiene todos los reponedores que no están asignados a ningún supervisor EN LA EMPRESA"""
        subquery = db.query(Supervision.reponedor_id).filter(Supervision.id_empresa == id_empresa)
        return db.query(Usuario).join(
    ...
    def get_supervisor_of_reponedor(self, db: Session, reponedor_id: int, id_empresa: int) -> Optional[Usuario]:
    ...
    def asignar_reponedor(self, db: Session, supervisor_id: int, reponedor_id: int, id_empresa: int) -> Supervision:
    ...
    def desasignar_reponedor(self, db: Session, supervisor_id: int, reponedor_id: int, id_empresa: int) -> None:
